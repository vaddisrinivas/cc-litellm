...
if [[ $? -ne 0 ]] || [[ "$RESULT" == *"gave up:true"* ]]; then
  log "user cancelled or dialog timed out — staying on Anthropic"
...
# Set env vars at system level (GUI + CLI)
...
# zshenv: propagates to new CLI sessions
ZSHENV="$HOME/.zshenv"
...
log "switch complete — proxy=http://localhost:4000"
